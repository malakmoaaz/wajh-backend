...
app.post('/api/simulations', async (req, res) => {
    try {
        const {
            caseId, surgeryName, confidence,
            initialLandmarks, modifiedLandmarks,
            surgicalPlan, resultImageData,
            aiRecommendation, goldenRatioData,
            mlProcedure, mlConfidence
        } = req.body;
...
app.get('/api/simulations', async (req, res) => {
    try {
        ...
        const sims  = await prisma.simulation.findMany({
            where, orderBy: { createdAt: 'desc' },
            select: {
                id: true, surgeryName: true, confidence: true,
                mlProcedure: true, mlConfidence: true,
                createdAt: true, caseId: true
            }
        });
        res.json(sims);
    } catch (e) { res.status(500).json({ message: e.message }); }
...
// ══════════════════════════════════════════════════════════════════
// CEPHALOMETRIC RULE ENGINE  (POST /api/analyze)
// ══════════════════════════════════════════════════════════════════
...
function analyze(deltas, pxPerMm) {
    ...
    if (mandAdv || chinAdv) {
        procs.push('Bilateral Sagittal Split Osteotomy (BSSO) — Advancement');
        classification = 'Angle Class II — Retrognathic Mandible';
        reasoning += 'Anterior movement of mandibular landmarks indicates mandibular deficiency. BSSO is the gold-standard procedure for advancing the mandible. ';
    }
    if (mandRet) {
        procs.push('BSSO — Mandibular Setback');
        classification = 'Angle Class III — Prognathic Mandible';
        reasoning += 'Posterior repositioning of mandibular landmarks detected. BSSO setback corrects mandibular prognathism. ';
    }
    if (chinAdv || chinInf || chinSup) {
        procs.push('Sliding Genioplasty');
        reasoning += 'Chin repositioning in multiple planes indicates genioplasty. ';
    }
    if (jawWidened) {
        procs.push('SARPE (Surgically Assisted Rapid Palatal Expansion)');
        reasoning += 'Lateral jaw widening pattern suggests transverse maxillary deficiency. ';
    }
    if (upperLipUp) {
        procs.push('Le Fort I Osteotomy — Superior Impaction');
        classification = 'Vertical Maxillary Excess';
        reasoning += 'Superior lip movement indicates vertical maxillary excess. ';
    }
    if (lowerLipDn && !mandAdv) {
        procs.push('Le Fort I Osteotomy — Inferior Repositioning');
        reasoning += 'Inferior lip movement may indicate maxillary vertical deficiency. ';
    }
    if (noseModif && !mandAdv && !mandRet) {
        procs.push('Rhinoplasty / Nasal Osteotomy');
        reasoning += 'Nasal landmark modifications suggest concurrent rhinoplasty. ';
    }

    const procedure = procs.length ? procs.join(' + ') : 'No Surgery Required';
    if (!procs.length) {
        classification = 'No major skeletal discrepancy detected';
        reasoning = 'Landmark movements are primarily in soft tissue. Conservative or orthodontic treatment may be sufficient.';
    }
    ...
    const measurements = deltas
        .filter(d => KEY_IDS.includes(d.id) && d.mag > (pxPerMm ? 1 : 3))
        .slice(0, 8)
        .map(d => {
            const direction = Math.abs(d.dxMm) >= Math.abs(d.dyMm)
    ...
    const targetLandmarks = deltas
        .filter(d => d.mag > (pxPerMm ? 1 : 3))
        .map(d => ({ id: d.id, x: Math.round(d.xAfter), y: Math.round(d.yAfter) }));
